...
Ubuntu の公式サイトから **Desktop Image** をダウンロードし、USBメモリに書き込みます。
...
| Windows PC   | Rufus を実行するため       |
| ISO ファイル | 先ほどダウンロードしたもの |
...
準備が整ったら、実際に Ubuntu をインストールしましょう。流れは「USBから起動する」「ブート順位を変える」「インストーラーに従う」の 3 つです。
